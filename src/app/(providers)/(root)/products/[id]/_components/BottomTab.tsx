...
function BottomTab({ productState }: BottomTabProps) {
  ...
  return (
    ...
    >
      <div className="md:max-w-[1240px] xs:mx-auto flex flex-col md:flex-row md:items-center p-[8px_20px_20px_20px] md:p-[0px] justify-between">
        {!isHidden && (
          <>
            <div className="flex justify-center items-center md:hidden">
              <button
                onClick={toggleVisibility}
                className="w-4 h-4 flex items-center justify-center pb-3 md:pb-0"
              >
                <Image src="/icons/bottomTab.svg" alt="hidden" width={16} height={16} />
              </button>
            </div>
            <p className="flex text-[16px] md:text-left md:text-[24px]">{product.title}</p>
          </>
        )}
