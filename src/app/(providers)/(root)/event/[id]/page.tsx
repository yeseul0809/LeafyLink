...
export default async function EventPage({ params }: EventPageProps) {
  ...
  const addHoursAndFormat = (timeString: string, hours: number) => {
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="h-5 justify-center items-center gap-2 inline-flex">
            ...
            <div className="justify-start items-center gap-1 flex">
              <div className="text-[#3bb873] text-sm font-semibold leading-tight">D-{dDay}</div>
              <div className="text-[#111111] text-xs font-normal leading-[18px]">
                {addHoursAndFormat(event.event_starttime, 9)} ~{' '}
                {addHoursAndFormat(event.event_endtime, 9)}
              </div>
            </div>
          </div>
        </div>
