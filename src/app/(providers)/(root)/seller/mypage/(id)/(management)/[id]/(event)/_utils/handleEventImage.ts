...
export const handleEventImage = (quillRef: React.RefObject<ReactQuill>) => {
  ...
  input.addEventListener('change', async () => {
    ...
    const { data, error } = await supabase.storage
      .from('event-image')
      .upload(`event_imgs/${fileNewName}`, file);
    ...
    const response = supabase.storage.from('event-image').getPublicUrl(`event_imgs/${fileNewName}`);
    if (!response.data) {
